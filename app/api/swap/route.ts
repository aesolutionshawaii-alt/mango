...
export async function POST(request: NextRequest) {
  try {
    const { profile, moodProfile, excludedMovies, seenMovie } = await request.json();
...
${moodProfile.map((m: { question: string; answer: string }) => `- ${m.question}: ${m.answer}`).join('\n')}`;
    ...
    const prompt = `You are a movie recommendation expert. The user already saw one of your recommendations, so suggest ONE replacement movie.

${contextPrompt}

The user already saw: ${seenMovie.title} (${seenMovie.year})

IMPORTANT: Include the director and top 3-4 main actors.

Suggest ONE different movie that fits the same mood and profile. Respond ONLY with valid JSON:
{
  "title": "Movie Title",
  "year": 2020,
  "streaming": "Netflix",
  ...
  "whyYou": "One sentence on why this fits YOU"
    ...
    return NextResponse.json({ error: 'Failed to swap movie' }, { status: 500 });
  }
}
